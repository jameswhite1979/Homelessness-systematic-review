*import data.
clear
import excel "C:\Users\wgprlc\OneDrive - Cardiff University\Homeless SR\DATA\Homelessness Clean included papers Stata.xlsx", sheet("All cause for Stata") cellrange(A1:BJ147) firstrow clear
...
meta forestplot if Sex != "Males and Females", subgroup(Sex Region) ///
	eform(Risk ratio) random(reml) noohetstats nullrefline ///
	columnopts(_K, title("Number of effects")) nooverall crop(0.5 16) xscale(range(.5 16)) xlabel(0.5 1 2 4 8 16, format(%2.1f)) columnopts(_lb, mask("[%4.2f",)) columnopts(_ub, mask("%4.2f]")) 

***********************************************************
*get interacton p-value
*Sex
